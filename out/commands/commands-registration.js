...
const delete_local_disk_image_1 = require("./delete-local-disk-image");
const registerCommands = () => {
    ...
    }), vscode_1.commands.registerCommand(`${app_context_1.AppContext.extName}.delete-local-disk-image`, (resource) => {
        console.log('Resource provided:', resource.fsPath);
        if (!resource) {
            vscode_1.window.showErrorMessage('No resource provided for deletion.');
            return;
        }
        (0, delete_local_disk_image_1.deleteLocalDiskImage)(resource);
    }));
};
